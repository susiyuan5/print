...
async function captureResponse(req, res, scroll) { try { const result = await capture(scroll ? req.body : {}); const captureId = randomUUID(); const state = browserStatus(); const record = { id: captureId, source: state.source, url: state.url, pageTitle: state.title, capturedAt: new Date().toISOString(), items: result.items.map((item) => ({ ...item, source: state.source })), warnings: result.warnings }; trendState.pendingCaptures.push(record); trendState.captureHistory.unshift({ id: captureId, source: state.source, url: state.url, capturedAt: record.capturedAt, rawItemCount: record.items.length, importedItemCount: 0, warnings: record.warnings }); await saveTrendState(); res.json({ ok: true, captureId, itemCount: result.items.length, warnings: result.warnings, url: state.url, pageTitle: state.title }); } catch (error) { res.status(400).json({ ok: false, error: error.message }); } }
