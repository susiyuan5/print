...
export function ColorsPage() {
  ...
  function submit(event: React.FormEvent) {
    ...
    dispatch({ type: "upsertPaint", paint: { id: editingId ?? `paint_${Date.now().toString(36)}`, name: form.name.trim(), brand: form.brand.trim() || undefined, code: form.code.trim() || undefined, hex: form.hex, finish: form.finish, notes: form.notes.trim() || undefined } });
    setEditingId(null);
    setForm(emptyForm);
  }
  ...
  return (
    <>
      <PageHeader title="颜色管理" description="维护涂料颜色、品牌编号、色值和质感。" />
      <section className="editor-layout">
        ...
        <section className="panel">
          <h2>颜色列表</h2>
          {data.paints.length === 0 ? <EmptyState title="还没有颜色" description="新增颜色后，可用于配色方案和喷涂记录。" /> : (
