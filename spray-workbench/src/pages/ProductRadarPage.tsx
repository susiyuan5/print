...
function breakdown(product: ProductOpportunity) {
  ...
  const deductions = [
    ...
    ["授权未知", product.licenseStatus === "unknown" ? 15 : 0],
  ] as [string, number][];
...
function confidence(product: ProductOpportunity) {
  let points = 10;
  if (product.sourceLinks.length) points += Math.min(30, product.sourceLinks.length * 10);
  if (product.evidenceNotes.length) points += Math.min(20, product.evidenceNotes.length * 10);
  if (product.licenseEvidence) points += 20;
...
export function ProductRadarPage() {
  ...
  const shown = useMemo(() => products.filter((product) => (market === "all" || product.markets.includes(market as ProductMarket)) && (quick === "all" || (quick === "shipping" && product.shippingScore >= 75) || (quick === "verified" && ["original", "commercial-license"].includes(product.licenseStatus) && !!product.licenseEvidence) || (quick === "blocked" && calc(product) === 0))).sort((a, b) => calc(b) - calc(a)), [products, market, quick]);
  ...
  return <>
    <PageHeader title="产品研发管线" description="发现产品 → 验证需求 → 核验授权 → 测试打印 → 测试销售 → 批准量产。" />
    ...
    <section className="editor-layout radar-layout">
      <form className="panel form-panel" onSubmit={save}>
        ...
        <div className="form-grid"><Field label="授权"><select value={form.license} onChange={(e) => setForm({ ...form, license: e.target.value as LicenseStatus })}>{["original", "commercial-license", "permission-required", "personal-use-only", "unknown"].map((item) => <option key={item}>{item}</option>)}</select></Field><Field label="IP 风险"><select value={form.ip} onChange={(e) => setForm({ ...form, ip: e.target.value as RiskLevel })}>{["low", "medium", "high", "blocked"].map((item) => <option key={item}>{item}</option>)}</select></Field><Field label="合规风险"><select value={form.compliance} onChange={(e) => setForm({ ...form, compliance: e.target.value as RiskLevel })}>{["low", "medium", "high", "blocked"].map((item) => <option key={item}>{item}</option>)}</select></Field></div>
        <Field label="授权证据"><textarea value={form.evidence} onChange={(e) => setForm({ ...form, evidence: e.target.value })} placeholder="批准量产前必填" /></Field>
        <h3>关联现有工作</h3>
